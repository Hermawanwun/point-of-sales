...
export default function Create({ categories }) {
    ...
    const createCodeSegment = (value, fallback, maxLength) => {
        const cleaned = (value || "")
            .toUpperCase()
            .replace(/[^A-Z0-9]/g, "");
    ...
    const barcodeTouchedRef = useRef(false);
    const skuTouchedRef = useRef(false);
    const barcodeSeedRef = useRef(
        `${Date.now().toString().slice(-10)}-${Math.floor(1000 + Math.random() * 9000)}`
    );
    const skuSeedRef = useRef(
        Math.random().toString(36).slice(2, 7).toUpperCase()
    );

    useEffect(() => {
        if (!barcodeTouchedRef.current) {
            const prefix = createCodeSegment(selectedCategory?.name, "PRD", 3);
            setData("barcode", `${prefix}-${barcodeSeedRef.current}`);
        }
    }, [selectedCategory]);

    useEffect(() => {
        if (!skuTouchedRef.current) {
            const prefix = createCodeSegment(data.title, "PRODUCT", 8);
            setData("sku", `SKU-${prefix}-${skuSeedRef.current}`);
        }
    }, [data.title]);
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <Input
                                    ...
                                    value={data.barcode}
                                    onChange={(e) => {
                                        barcodeTouchedRef.current = true;
